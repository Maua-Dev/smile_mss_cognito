...
from src.shared.infra.dtos.db_base import DbBaseModel
...
class CognitoUserDTO(DbBaseModel):
    ...
    def __init__(self, data: dict):
        ...
        def defaultDataTemplate(field, value): return {
        ...
        for i in self.to_dict():
            if self.__getattribute__(i) is not None and i != 'password' and i != 'userSub':
                self.userAttributes.append(defaultDataTemplate(
                    i, str(self.__getattribute__(i))))
    ...
    def fromKeyValuePair(data: dict):
        ...
        for i in data:
            ...
            try:
                value = i['Value'] if (field != 'accessLevel' and field != 'role') else ACCESS_LEVEL(
                    i['Value']) if field == 'accessLevel' else ROLE(i['Value'])
                fixedField = field if field not in CognitoUserDTO.changedFieldNames else CognitoUserDTO.changedFieldNames[
                    field]
